...
struct JSONValue
{
    ...
    {
        return (this.val).match!((bool x) => hashOf(x), (long x) => hashOf(x),
                (double x) => hashOf(x), (const(char)[] x) => hashOf(x));
    }
    ...
    int opCmp(JSONValue other) const pure nothrow
    {
        if (this == other)
            return 0;
        auto ret = match!((bool a, bool b) => a < b ? -1 : 1, (long a,
                long b) => a < b ? -1 : 1, (double a, double b) => a < b ? -1 : 1,
                (const(char)[] a, const(char)[] b) => a < b ? -1 : 1, (_a, _b) => -1)(this.val,
                other.val);
        return ret;
    }
    ...
    {
        return match!((bool a, bool b) => a == b, (long a, long b) => a == b,
                (double a, double b) => a == b, (const(char)[] a,
                    const(char)[] b) => a == b, (_a, _b) => false)(this.val, other.val);
    }

    string toString() const
    {
        return (this.val).match!((bool x) => x.to!string,
                (long x) => x.to!string, (double x) => x.to!string, (const(char)[] x) => x.idup,);
    }

    ulong getType() const
    {
        return (this.val).match!((bool x) => 0, (long x) => 1, (double x) => 2,
                (const(char)[] x) => 3);
    }

    ubyte[] toBytes() const
    {
        return (this.val).match!((bool x) {
            ubyte[] ret = new ubyte[1];
            ret[0] = x ? 1 : 0;
            return ret;
        }, (long x) { ubyte[] ret = new ubyte[8]; i64_to_le(x, ret.ptr); return ret; }, (double x) {
