...
struct BasicQuery {
    ...
    string toString()
    {
        return (*this.expr).match!(
            (Value x) => x.toString(),
            (KeyValue x) => x.toString(),
            (UnaryKeyOp x) => x.toString()
        );
    }
    ...
    bool opEquals(const BasicQuery other) const
    {
        return match!(
            ...
            (const KeyValue a, const KeyValue b) => a == b,
            (const UnaryKeyOp a, const UnaryKeyOp b) => a == b,
            (_a, _b) => false,
...
alias QueryExpr = SumType!(
    BasicQuery,
    Tuple!(Key, "lhs", This*, "rhs"),
    Tuple!(UnaryLogicalOp, "op", This*, "negated"),
    Tuple!(This*, "sub"),
...
struct Query {
    ...
    {
        return (*this.expr).match!(
            (BasicQuery x) => x.toString(),
            (ComplexKeyValue x) => x.lhs.key  ~ "=" ~ Query(*x.rhs).toString(),
            (NotQueryExpr x) => "!" ~ Query(*x.negated).toString(),
            (SubQueryExpr x) => "(" ~ Query(*x.sub).toString() ~ ")",
            (ComplexQuery x) => Query(*x.lhs).toString()  ~ cast(string)x.op ~ Query(*x.rhs).toString(),
        );
    }
    ...
    {
        return match!(
            (const BasicQuery a, const BasicQuery b) => a == b, 
            (const ComplexKeyValue a, const ComplexKeyValue b) => a.lhs == b.lhs && a.rhs == b.rhs,
            (const NotQueryExpr a, const NotQueryExpr b) => a.op == b.op && a.negated == b.negated,
            (const SubQueryExpr a, const SubQueryExpr b) => a.sub == b.sub,
            (const ComplexQuery a, const ComplexQuery b) => a.op == b.op && a.lhs == b.lhs && a.rhs == b.rhs,
            (_a, _b) => false,
        )(*this.expr, *other.expr);
    }
...
QueryExpr * parseQueryRecurse(Tokenized tokens)
{
    ...
    auto isParenthesis1 = (Operators v) => v.match!(
        (Parenthesis x) => true,
        (_x) => false,
    );
    auto isParenthesis = (TokenizedTypes v) => v.match!(
        (Operators x) => isParenthesis1(x),
        (_x) => false
    );
    auto matchOp = (Operators v, Tokenized toks) => v.match!(
        (Parenthesis x) {
            if(toks.length == 1)
    ...
    auto isValueOp1 = (Operators v) => v.match!(
        (ValueOp x) => true,
        (_x) => false,
    );
    auto isValueOp = (TokenizedTypes v) => v.match!(
        (Operators x) => isValueOp1(x),
        (_x) => false
    );

    auto isString = (TokenizedTypes v) => v.match!(
        (Operators x) => false,
        (string x) => true
    );
    
    return tokens[0].match!(
        (Operators x) => matchOp(x, tokens),
        (string x) {
            /// Value or UnaryKeyOp
            if(tokens.length < 3){
