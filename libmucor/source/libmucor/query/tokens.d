...
T getInner(T)(TokenizedTypes v)
{
    ...
    static if(staticIndexOf!(T, Operators.Types) != -1){
        auto matchOp = (Operators o) => o.tryMatch!(
            (T x) => x
        );
        return v.tryMatch!(
            (Operators x) => matchOp(x)
        );
    } else {
        return v.tryMatch!(
            (string x) => x
...
struct Tokenized {
    ...
    string toString(){
        auto matchOp = (Operators x) => match!(
            (Parenthesis x) => cast(string) x,
            (ValueOp x) => cast(string) x,
            (BinaryLogicalOp x) => cast(string) x,
            (UnaryLogicalOp x) => cast(string) x,
            (KeyOp x) => cast(string) x,
        )(x);
        string ret;
        foreach(item; this.tokens){
            item.match!(
                (string x) => ret ~= x,
                (Operators x) => ret ~= matchOp(x)
            );
        }
        return ret;
    }
    ...
    {
        return Tokenized(
            this.tokens[slice[0]..slice[1]], 
            this.idxs[slice[0]..slice[1]],
            original);
    }
    ...
    Tokenized sliceToNextClosingParenthesis(){
        ...
        foreach (tok; tokens)
        {
            auto matchOp = (Operators x) => x.match!(
                (Parenthesis x) => x == Parenthesis.Left ? 1 : -1,
                (_x) => 0,
            );
            tok.match!(
                (Operators x) {
    ...
    auto getFrontInner(T)(){
        try {
            ...
            static if(is(T == string))
                ...
                queryErr(original, this.idxs[0], format("Unexpected token, Expected one of \"%s\"", cast(string[])[EnumMembers!T]));
            return T.init;
        }
    }
