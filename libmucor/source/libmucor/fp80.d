...
struct FP80
{
    nothrow @nogc pure @trusted
    {
        ...
        ushort exponent() const
        {
            return exp_sign & 0x7fff;
        }

        bool sign() const
        {
            return (exp_sign & 0x8000) != 0;
        }

        ref FP80 opAssign(FP80 ld) return
        {
            mantissa = ld.mantissa;
            exp_sign = ld.exp_sign;
            return this;
        ...
        {
            static if (op == "-")
                return FP80(mantissa, exp_sign ^ 0x8000);
            else
                static assert(false, "Operator `" ~ op ~ "` is not implemented");
        }

        T opCast(T)() const @trusted
        {
            static if (is(T == bool))
                return mantissa != 0 || (exp_sign & 0x7fff) != 0;
            ...
            else static if (is(T == real))
            {
                ...
                static assert(false, "usupported type");
        }

        // a qnan
        static FP80 nan()
        {
            return FP80(0xC000000000000000UL, 0x7fff);
        }

        static FP80 infinity()
        {
            return FP80(0x8000000000000000UL, 0x7fff);
        }
        ...
        static FP80 max()
        {
            return FP80(0xffffffffffffffffUL, 0x7ffe);
        }

        static FP80 min_normal()
        {
            return FP80(0x8000000000000000UL, 1);
        }

        static FP80 epsilon()
        {
            return FP80(0x8000000000000000UL, 0x3fff - 63);
        }
        ...
        static uint max_10_exp()
        {
            return 4932;
        }

        static uint min_10_exp()
        {
            return -4932;
        }
