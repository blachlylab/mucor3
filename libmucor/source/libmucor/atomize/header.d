        ...
        for (auto i = 0; i < header.hdr.nhrec; i++)
        {
            ...
            switch(hrec.recType){
                case HeaderRecordType.Info:
                    ...
                    if(hrec.getID() == "ANN") this.isAnn[idx] = true;
            ...
            if(this.remapIdxs[i] != -1){
                ...
                if(this.isInfo[i]) {
                    ...
                    ret ~= "\tINFO/%s:(id: %d, newId: %d, type: %s, num: %s, byAllele: %s, isAnn: %s),\n"
                        .format(name, i, newIdx, finfo.t, finfo.n, this.isByAllele[i], this.isAnn[i]);
                } else {
                    HdrFieldInfo finfo;
                    string name;
                    if(this.isByAllele[i]){
                    ...
                    ret ~= "\tFMT/%s:(id: %d, newId: %d, type: %s, num: %s, byAllele: %s),\n"
                        .format(name, i, newIdx, finfo.t, finfo.n, this.isByAllele[i]);
                }
