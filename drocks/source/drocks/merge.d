...
alias FullMergeFn = char* function (void*, const(char)* key, size_t key_length, const(char)* existing_value, size_t existing_value_length, const(char*)* operands_list, const(size_t)* operands_list_length, int num_operands, ubyte* success, size_t* new_value_length);
alias MergeDtr = void function (void*);
alias PartialMergeFn = char* function (void*, const(char)* key, size_t key_length, const(char*)* operands_list, const(size_t)* operands_list_length, int num_operands, ubyte* success, size_t* new_value_length);
...
char* AppendFullMerge(
    void* state, 
    const(char)* key, 
    size_t key_length, 
    const(char)* existing_value, 
    size_t existing_value_length, 
    const(char*)* operands_list, 
...
auto createAppendMergeOperator() {
    ...
    return rocksdb_mergeoperator_create(state, &AppendDtr, &AppendFullMerge, &AppendPartialMerge, null, &AppendName);
}
