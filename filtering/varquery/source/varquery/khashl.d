...
template khashl(KT, VT, bool kh_is_map = true, bool cached = false, bool useGC = true)
{
    ...
    struct khashl        // @suppress(dscanner.style.phobos_naming_convention)
    {
        ...
        bool opBinaryRight(string op)(KT key) const
        if (op == "in") 
        {
            Bucket ins;
            ins.key = key;
            static if(cached) ins.hash = __hash_func(ins.key); //cache the hash
            auto x = kh_get(&this, ins);
            return x == kh_end(&this) ? false : true;
        }
    ...
    void kh_clear(kh_t* h)
    {
      if (h && h.used)
      {
        ...
        memset(h.used.ptr, 0, __kh_fsize(n_buckets) * khint32_t.sizeof); 
        h.count = 0; 
      }
    }
    ...
    int kh_resize(kh_t *h, khint_t new_n_buckets)
	{
		...
		// memset(new_used.ptr, 0, __kh_fsize(new_n_buckets) * khint32_t.sizeof);
		// if (!new_used) return -1; /* not enough memory */
		n_buckets = h.keys.ptr? 1U<<h.bits : 0U;
		if (n_buckets < new_n_buckets) { /* expand */
			h.keys.length = new_n_buckets;
