...
InvertedIndex*[2] processVcfRaw(R)(R aRange, R bRange, string prefix)
{
    ...
    writeAllSets(aAFVar, bAFVar, samVarKeys ~ ["AF"], stats.afVarCounts[],
            prefix, "unique.AF.sample.var");
...
void processVcfFiltered(R)(R aRange, R bRange, string queryStr,
        InvertedIndex*[2] indexes, string prefix)
{
    Set aVar, bVar, aSamVar, bSamVar, aAFVar, bAFVar;
    ...
    aRange.query(indexes[0], queryStr).tee!(x => stats.aCount++)
        .tee!(x => aVar.insert(subset(x, varKeys)))
        .tee!(x => aSamVar.insert(subset(x, samVarKeys)))
        .each!(x => aAFVar.insert(getAFValue(x, 0.01)));

    bRange.query(indexes[1], queryStr).tee!(x => stats.bCount++)
        .tee!(x => bVar.insert(subset(x, varKeys)))
        .tee!(x => bSamVar.insert(subset(x, samVarKeys)))
...
void writeSet(Set s, string[] fields, ulong* count, string fn)
{
    ...
    s.byKey
        .tee!(x => (*count)++)
        .map!(x => *cast(Asdf*)&x)
        .to_table(fields).each!(x => f.writeln(x));
}
